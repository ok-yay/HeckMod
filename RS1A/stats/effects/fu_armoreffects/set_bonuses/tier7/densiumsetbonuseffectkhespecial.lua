...
function update(dt)
	if not checkSetWorn(self.setBonusCheck) then
		status.removeEphemeralEffect("convert_energy-health_10_1-1_inefficientover_special")
		status.removeEphemeralEffect("devaarmorpenalty")
		effect.expire()
	else
		status.addEphemeralEffect("convert_energy-health_10_1-1_inefficientover_special")
