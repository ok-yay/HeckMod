...
function init()
    ...
    self.naming = config.getParameter("itemName")
    self.forceWalk = config.getParameter("forceWalk", false)
    self.realKnockback = self.knockback
...
function update(dt, fireMode, shiftHeld)
    ...
    if not self.active and fireMode == "primary" and self.cooldownTimer == 0 and
        status.resourcePositive("shieldStamina") then
    ...
    if self.active then
        ...
        if (fireMode ~= "primary" and self.activeTimer >= self.minActiveTime) or
            not status.resourcePositive("shieldStamina") then
            lowerShield()
        end
    end
...
function refreshPerfectBlock()
    local perfectBlockTimeAdded = math.max(0, math.min(status.resource("perfectBlockLimit"),
        self.perfectBlockTime - status.resource("perfectBlock")))
